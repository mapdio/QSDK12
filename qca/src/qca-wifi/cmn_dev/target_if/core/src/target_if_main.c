					   ...
					   uint8_t num_links, uint8_t grp_id)
{
	...
	if (params.pdev_id >= num_links || !pdev_list[params.pdev_id]) {
		target_if_err("Invalid pdev_id or pdev_list entry is NULL\n");
		return QDF_STATUS_E_INVAL;
	}

	max_num_ml_peers = pdev_list[params.pdev_id]->
				pdev_objmgr.max_num_ml_peers;

	for (idx = 0; idx < num_links; idx++) {
		if (pdev_list[idx] == NULL) {
			target_if_err("PDEV is NULL for idx: %d", idx);
			return QDF_STATUS_E_INVAL;
		}
		 ...
		 */
		if (max_num_ml_peers >
				pdev_list[idx]->pdev_objmgr.max_num_ml_peers){
			max_num_ml_peers =
				pdev_list[idx]->pdev_objmgr.max_num_ml_peers;
